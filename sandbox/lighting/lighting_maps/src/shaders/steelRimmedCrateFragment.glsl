...
void main() {
    // ambient
    vec3 ambientLight = light.ambientLevel * light.color * vec3(texture(material.diffuseMap, vOutTexCoord));
    ...
    vec3 diffusionLight =
        light.diffuseLevel * light.color * facingLightDotProd * vec3(texture(material.diffuseMap, vOutTexCoord));
    ...
    vec3 specularLight = light.specularLevel * light.color * (specularDotProd * vec3(texture(material.specularMap, vOutTexCoord)));
